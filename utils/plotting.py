  ...
  for indir in args.indir:
    ...
    for filename in filenames:
      try:
        ...
        task, method = filename.relative_to(indir).parts[:-1]
        continue
...
def load_run(filename, indir, args):
  ...
  try:
    ...
    return
  if not args.yaxis in df:
    args.yaxis = 'train/return' # handle dmc
  try:
    df = df[[args.xaxis, args.yaxis]].dropna()
  except KeyError:
    print(f'Key error: {filename}')
    return
...
def figure(runs, args):
  all_tasks = [r.task for r in runs if r.xs is not None]
  task_names = [p.pattern for p in args.tasks]
  tasks = sorted(set(all_tasks), key=lambda x: task_names.index(x) if x in task_names else 0)
...
def plot(task, ax, runs, args):
  ...
  for index, method in enumerate(methods):
    ...
    if any(r.xs is None for r in relevant):
      ...
      ylim = [min(ylim[0], ys.min()), max(ylim[1], ys.max())]
  if args.xlim:
    xlim[0] = max(args.xlim[0], xlim[0])
    xlim[1] = min(args.xlim[1], xlim[1])
  ...
  if args.ylim:
    ylim[0] = args.ylim[0]
    # Round up ylim
    if args.ylim[1] < ylim[1]:
      ylim[1] = (ylim[1] // 100 + 1) * 100
    else:
      ylim[1] = args.ylim[1]
    ax.set_ylim(ylim)
    ticks = sorted({*ax.get_yticks(), *ylim})
    ticks = [x for x in ticks if ylim[0] <= x <= ylim[1]]
    ax.set_yticks(ticks)


def smoothing(ys, weight):
  last = ys[0]
  smoothed_ys = np.zeros_like(ys)
  for i, y in enumerate(ys):
    smoothed_y = weight * last + (1 - weight) * y
    smoothed_ys[i] = smoothed_y
    last = smoothed_y
  return smoothed_ys
...
def curve_individual(index, method, ax, runs, args):
  if args.bins:
    for index, run in enumerate(runs):
      xs, ys = binning(run.xs, smoothing(run.ys, args.smoothing), args.init_bin, args.bins, np.nanmean)
...
def curve_std(index, method, ax, runs, args):
  if args.bins:
    for index, run in enumerate(runs):
      xs, ys = binning(run.xs, run.ys, args.init_bin, args.bins, np.nanmean)
      runs[index] = run._replace(xs=xs, ys=ys)
  ...
  if args.bins:
    ...
    xs, ys = binning(xs, ys, args.init_bin, args.bins, reducer)
    ys, std = ys.T
...
def binning(xs, ys, init_bin, bins, reducer):
  binned_xs = np.arange(xs.min() + init_bin, xs.max() + init_bin, bins) # 1e-10
  binned_ys = []
...
def parse_args():
  ...
  parser.add_argument('--init_bin', type=float, default=0)
